...
class  adminback
{
    ...
    function add_product($data)
    {
        ...
        $pdt_sound_name = $_FILES['pdt_sound']['name'];
        $pdt_sound_size = $_FILES['pdt_sound']['size'];
        $pdt_sound_tmp = $_FILES['pdt_sound']['tmp_name'];
        ...
        if (($img_ext == "jpg" || $img_ext == 'jpeg' || $img_ext == "png") &&
            ...
            if (($sound_ext == "mp3" || $sound_ext == 'm4a') &&
                $pdt_sound_size <= 2e+6
            ) {
                $query = "INSERT INTO `products`(`pdt_name`, `pdt_price`, `pdt_des`, `pdt_ctg`, `pdt_img`, `pdt_sound`, `pdt_status`) VALUES ('$pdt_name',$pdt_price,'$pdt_des',$pdt_ctg,'$pdt_img_name','$pdt_sound_name',$pdt_status)";
                ...
                $msg = "Sound file should be mp3 or m4a format and size should not be larger than 2MB";
                return $msg;
            }
        } else {
            $msg = "Image file should be jpg or png format, size should not be larger than 2MB, and dimensions should be less than 271x271 px";
            return $msg;
        }
    }
    ...
    function update_product($data)
    {
        ...
        if (($img_ext == "jpg" || $img_ext == 'jpeg' || $img_ext == "png") && $pdt_img_size <= 2e+6 && $width < 271 && $height < 271 &&
            ($sound_ext == "mp3" || $sound_ext == 'm4a') && $pdt_sound_size <= 2e+6
        ) {
            ...
            $msg = "Invalid image or sound format, size, or dimensions";
            return $msg;
        }
    }
    ...
    function confirm_order($post, $session)
    {
        ...
        foreach ($session as $key) {
            ...
            $query = "INSERT INTO `order_details`(`user_id`,`product_id`, `product_name`,`amount`,`uses_coupon`, `order_status`, `trans_id`, `Shipping_mobile`, `shiping`, `order_time`) VALUES ($user_id,$pdt_id,'$pdt_name', $pdt_price,'$coupon', $order_status,'$trans_id','$mobile','$shiping',NOW())";
            $result = mysqli_query($this->connection, $query);
            unset($_SESSION['cart']);
            header("location:exist_order.php");
        }
    ...
    {
        $query = "SELECT * FROM products WHERE pdt_id = $product_id";
        $result = mysqli_query($this->connection, $query);
